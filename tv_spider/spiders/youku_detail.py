...
class YoukuDetailSpider(scrapy.Spider):
    ...
    def parse_tv_detail_link(self, response):
        ...
        if response.status >= 200 and response.status < 300 and desc_link:
            ...
            self.db.videos.update_one({'resources': {'$elemMatch': {'source': resource.get('source'), 'id': resource.get('id')}}}, {'$set': {'resources.$.has_crawl_detail': True, 'resources.$.status': tv_status.UNAVAILABLE}})
